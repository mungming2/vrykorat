<?php
 ...
 if($type == 1){
	 ...
	 $sql = "SELECT @rank:=@rank+1 AS no, stock_id, CONCAT(cus_title, cus_firstname,' ', cus_lastname) AS cus_name, insure_date, car_number, stock_date, insure_net, insure_total2, insure_duty, insure_tex FROM insure_payment, (SELECT @rank := 0) r WHERE stock_date = '$date'".$where_search." ORDER BY stock_id ASC";
	 ...
	 foreach ($res_report as $key => $value) {
		 ...
		 $sql_pay = "SELECT stock_id, balance ,arrears ,payment_type FROM payment WHERE stock_id = '".$value['stock_id']."' ORDER BY count ASC";
	 ...
	 //แสดงกรมธรรม์จากวันที่หรือคำค้นหา
	 $sql = "SELECT @rank:=@rank+1 AS no, no AS as_no, stock_id, selectagent, CONCAT(cus_title, cus_firstname,' ', cus_lastname) AS cus_name, insure_date, car_number, inform_code, inform_date, insure_net, insure_total2, insure_duty, insure_tex FROM insure_payment2, (SELECT @rank := 0) r WHERE inform_date = '$date'".$where_search." ORDER BY stock_id ASC";
	 $query = mysqli_query($conn,$sql);
	 $res_report = mysqli_fetch_all($query,MYSQLI_ASSOC);
	 ...
	 foreach ($res_report as $key => $value) {
		 ...
		 $sql_pay = "SELECT stock_id, balance ,arrears ,payment_type FROM payment WHERE stock_id = '".$value['as_no']."' ORDER BY count ASC";
		 $query_pay = mysqli_query($conn,$sql_pay);
		 $res_pay = mysqli_fetch_all($query_pay,MYSQLI_ASSOC);
...
<body >
	<div class="container-fluid">
		...
		<div class="mt-5 h3 font-weight-light row" style="font-size: 16px;">
			<div class="col-4">
			...
			<div class="col-8">
				<p>ได้แก่กรมธรรม์ <b class="mx-4"><?=$show_stock1?></b></p>
			</div>
			<?php } ?>
			...
			<div class="col-8">
				<p>ได้แก่กรมธรรม์ <b class="mx-4"><?=$show_stock2?></b></p>
			</div>
