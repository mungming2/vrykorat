...
session_start();
	...
	echo $sql = "UPDATE cheque SET id='$id',receive_cheque_date='$receive_cheque_date',bank='$bank',bank_branch='$bank_branch',cheque_date='$cheque_date',payment_total='$payment_total',owner_payment='$owner_payment',stock_id='$stock_id',list='$list',list_detail='$list_detail',owner_cheque='$owner_cheque',cheque_status='$cheque_status',cheque_status_detail='$cheque_status_detail' WHERE id = '$id'";
	$query = mysqli_query($conn,$sql);
	echo $errMsg = mysqli_error($conn);
	echo "<br>";

	if($query){
		echo "<meta http-equiv='refresh' content='0;url=../main.php?status=success&page=income-edit2&search=".$id."'>";
	}else{
		echo "<meta http-equiv='refresh' content='0;url=../main.php?page=income-edit2&status=fail&msg=".$errMsg."&search=".$id."'>";
	}
