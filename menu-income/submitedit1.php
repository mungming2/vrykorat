...
session_start();
	...
	echo $sql = "UPDATE income SET bill_no='$bill_no',receive_cheque_date='$receive_cheque_date',type1='$type1',inform_id='$inform_id',list_income='$list_income',status_payment='$status_payment',payment_total='$payment_total',bank='$bank',bank_branch='$bank_branch',payment_date='$payment_date',cheque_number='$cheque_number' WHERE id = '$id'";
	$query = mysqli_query($conn,$sql);
	echo $errMsg = mysqli_error($conn);
	echo "<br>";

	if($query){
		echo "<meta http-equiv='refresh' content='0;url=../main.php?status=success&page=income-edit1&search=".$id."'>";
	}else{
		echo "<meta http-equiv='refresh' content='0;url=../main.php?page=income-edit1&status=fail&msg=".$errMsg."&search=".$id."'>";
	}
